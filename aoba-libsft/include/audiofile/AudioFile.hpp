...
bool AudioFile<T>::decodeWaveFile (std::vector<uint8_t>& fileData)
{
    ...
    if ((numBytesPerSecond != (int)(numChannels * sampleRate * bitDepth) / 8) || (numBytesPerBlock != (numChannels * numBytesPerSample)))
    {
...
bool AudioFile<T>::decodeAiffFile (std::vector<uint8_t>& fileData)
{
    ...
    if ((soundDataChunkSize - 8) != totalNumAudioSampleBytes || totalNumAudioSampleBytes > (int)(fileData.size() - samplesStartIndex))
    {
...
bool AudioFile<T>::saveToWaveFile (std::string filePath)
{
    ...
    if ((unsigned long)fileSizeInBytes != (fileData.size() - 8) || dataChunkSize != (getNumSamplesPerChannel() * getNumChannels() * (bitDepth / 8)))
    {
...
int AudioFile<T>::getIndexOfString (std::vector<uint8_t>& source, std::string stringToSearchFor)
{
    ...
    for (int i = 0; i < (int)source.size() - stringLength;i++)
    {
