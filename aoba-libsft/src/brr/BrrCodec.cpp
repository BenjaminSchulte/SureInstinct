...
template <typename T>
inline static T clamp(T x, uint N) {
  T low = -(1 << (N - 1));
  T high = (1 << (N - 1)) - 1;
...
template <typename T>
inline static T clamp16(T x) { return clamp<T>(x, 16); }

template <typename T>
inline static T clamp15(T x) { return clamp<T>(x, 15); }


template <int16_t G4, int16_t G3, int16_t G2>
inline static uint8_t test_gauss (const int16_t* ls) {
  int32_t s;
  s =  int32_t(G4 * ls[0]) >> 11;
  s += int32_t(G3 * ls[1]) >> 11;
  s += int32_t(G2 * ls[2]) >> 11;
...
inline static uint8_t test_overflow (const int16_t* ls) {
  uint8_t r;
...
void BrrCodec::reset() {
  ...
  mLoopEnabled = false;
  mGaussEnabled = false;
  mUserPitchEnabled = false;
  mHeaderInclude = false;
  mOldLoop = false;

  mPitchStepBase = 0x1000;

  mInputSampleRate = 32000;
  mOutputSampleRate = 32000;
...
void BrrCodec::decode() {
  ...
  if (!mGaussEnabled) {
    // 7.8125 = 32000 / 0x1000
    mOutputSampleRate = uint32_t(double(mPitchStepBase) * 7.8125 + 0.5);
  }
  ...
  mBrrData.resize(((mBrrData.size() + 8) / 9) * 9, 0);
  mBrrData.end()[-9] |= 1;

  std::vector<uint8_t>::const_iterator data = mBrrData.begin();
  int16_t sample[8];  // 4 samples stored twice
  int16_t last_sample[2];
  uint8_t header = 0;
  uint samp_i = 0;
  uint brr_counter = 1;  // --1 == 0
  int32_t pitch = 0x3000;  // decode 4 samples
  ...
  while (true) {
    while (pitch >= 0) {
      ...
      if (--brr_counter == 0) {
        if (header & 1) {
          const int16_t* samp = &sample[samp_i];
          int32_t s;
          for (int i=2; i>=0; i--) {
            s = samp[i];
            s <<= 1;
            mWavData.push_back(s);
          }
          if (header & 2) {
            mLoopEnabled = true;
          }
      ...
      if (brr_counter & 1) {
        ++data;
        s = int32_t((samp & 0x0F) ^ 8) - 8;
      } else {
        s = int32_t((samp >> 4) ^ 8) - 8;
      }
      ...
      s = int16_t(s << 1) >> 1;  // wrap to 15 bits, sign-extend to 16 bits
    ...
    if (mGaussEnabled) {
      const int32_t p = pitch >> 4;
      const int32_t np = -p;
      const int16_t* G4 = &gauss_table[-1 + np];
      const int16_t* G3 = &gauss_table[255 + np];
      const int16_t* G2 = &gauss_table[512 + p];
      const int16_t* G1 = &gauss_table[256 + p];
      ...
      s =  int32_t(G4[0] * samp[3]) >> 11;
      s += int32_t(G3[0] * samp[2]) >> 11;
      s += int32_t(G2[0] * samp[1]) >> 11;
      s =  int16_t(s << 1) >> 1;
      s += int32_t(G1[0] * samp[0]) >> 11;
      s = clamp15(s);

      s = (s * 0x07FF) >> 11;  // envx
      s = (s * 0x7F) >> 7;  // volume

      pitch += mPitchStepBase;
    } else {
      s = samp[3];
      pitch += 0x1000;
    }
...
void BrrCodec::encode() {
  ...
  if (mLoopStart >= mWavData.size()) {
    mLoopStart = 0;
    mLoopEnabled = false;
  }

  /*if (mLoopEnabled) {
    uint32_t start_align = (16 - (mLoopStart & 15)) & 15;
    uint32_t loop_size = mWavData.size() - mLoopStart;
    uint32_t end_align = loop_size;

      qDebug() << "REALIGN LOOP" << end_align & 15;
    ...
    // also include the mLoopStart alignment
    end_align += start_align;

    if (end_align != 0) {
      mWavData.resize(mWavData.size() + end_align, 0);

      const int16_t* src = &mWavData[mLoopStart];
      int16_t* dst = &mWavData[mLoopStart + loop_size];
      const int16_t* end = dst + end_align;

      while (dst != end) {
        *dst++ = *src++;
      }

      // 16-sample align mLoopStart
      mLoopStart += start_align;
    }
  }
  else {*/
    mWavData.resize(((mWavData.size() + 15) / 16) * 16, 0);
  //}

  if (mHeaderInclude) {
    mBrrData.push_back(uint8_t(mLoopStart / 16u * 9u));
    mBrrData.push_back(uint8_t((mLoopStart / 16u * 9u) >> 8));
  }

  const float base_adjust_rate = 0.0004;
  float adjust_rate = base_adjust_rate;
  const uint32_t loop_block = mLoopStart / 16u;
  const uint32_t wimax = mWavData.size() / 16u;
  uint32_t wi = 0;
  int16_t best_samp[18];
  ...
  while (wi != wimax) {
    ...
    for (uint8_t filter = 0; filter <= 3; ++filter) {
      if (filter != 0) {
        if ((wi == 0) || ( (wi == loop_block) && mOldLoop ) )
        continue;
      }
      ...
      for (uint8_t range = 12; range >= 1; --range) {
        const int32_t rhalf = (1 << range) >> 1;
        double blk_err = 0;
        uint8_t blk_data[16];

        for (uint n = 0; n < 16; ++n) {
          int16_t* blk_ls = blk_samp + n;
          int32_t filter_s;
          ...
          int32_t s1 = int16_t(xs & 0x7FFF);
          int32_t s2 = int16_t(xs | 0x8000);
        ...
        // BRR accumulates to 17 bits, saturates to 16 bits, and then wraps to 15 bits
          ...
          if (filter >= 2) {
            s1 = clamp16(s1 + filter_s);
            s2 = clamp16(s2 + filter_s);
          } else {
            // don't clamp - result does not overflow 16 bits
            s1 += filter_s;
            s2 += filter_s;
        ...
        if (blk_err < best_err) {
          ...
          best_data[0] = (range << 4) | (filter << 2) | (uint8_t(mLoopEnabled) << 1);
    ...
    for (uint n = 0; n < 16; ++n) {
      uint8_t b = test_overflow(best_samp + n);
    ...
    if (overflow) {
      ...
      mTotalError += best_err;

      if (best_err < mMinError) {
        mMinError = best_err;
      }

      if (best_err > mMaxError) {
        mMaxError = best_err;
      }

      mBrrData.insert(mBrrData.end(), best_data, best_data + 9);
  ...
  if (wimax == 0) {
    ...
    mAverageError = mTotalError / wimax;
  }

  //if (mBrrData.empty() || !mLoopEnabled)
  // mBrrData.insert(mBrrData.end(), 9, 0);

  //mBrrData.end()[-9] |= (uint8_t(mLoopEnabled) << 1) | 1;
  mBrrData.end()[-9] |= 1;

  if (!mUserPitchEnabled) {
    // 0.128 = 0x1000 / 32000
    uint x = uint(double(mInputSampleRate) * 0.128 + 0.5);
...
// -----------------------------------------------------------------------------
void BrrCodec::toBrrSample(BrrSample *sample) const {
  int size = mBrrData.size();
  sample->clear();
  ...
  for (int i=0; i<size; i+=9) {
    ...
    sample->appendChunk()->readFrom(&data);
  }
}

// -----------------------------------------------------------------------------
void BrrCodec::loadFromBrr(const BrrSample *sample, int loopFromChunk) {
  int size = sample->streamLengthInBytes();
  ...
  for (int index=0; index<size; index++) {
    mBrrData.push_back(data[index]);
  }

  mLoopEnabled = (loopFromChunk != -1);
  mLoopStart = loopFromChunk * 16;
  free(data);
}

// -----------------------------------------------------------------------------
void BrrCodec::toWave(SampleData *sample, int sampleRate) const {
  int size = mWavData.size();

  sample->allocate(1, sampleRate, size);
  for (int i=0; i<size; i++) {
    sample->sample(0, i) = mWavData[i] / (double)0x7FFF;
  }
}

// -----------------------------------------------------------------------------
void BrrCodec::loadFromWave(const SampleData *data, double loopFromOffset) {
  reset();

  int numSamples = data->samplesPerChannel();
  ...
  if (loopFromOffset >= 0) {
    mLoopEnabled = true;
    mLoopStart = loopFromOffset / data->duration() * numSamples;
    mLoopStart = ((mLoopStart + 8) / 16) * 16;
  }

  for (int i=0; i<numSamples; i++) {
    double sample = 0;
    for (int j=0; j<numChannels; j++) {
      sample += data->sample(j, i);
    }

    mWavData.push_back((sample / numChannels) * 0x7FFF);
  }
}