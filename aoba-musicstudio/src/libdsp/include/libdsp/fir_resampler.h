...
protected:
	...
	InternalFirResampler( int width, sample_t* );
...
template<int width>
class FirResampler : public InternalFirResampler {
	BOOST_STATIC_ASSERT( width >= 4 && width % 2 == 0 );
	short impulses [max_res] [width];
public:
	FirResampler() : InternalFirResampler( width, impulses [0] ) { }
...
inline void InternalFirResampler::write( long count )
{
	write_pos += count;
...
template<int width>
int FirResampler<width>::read( sample_t* out_begin, blargg_long count )
{
