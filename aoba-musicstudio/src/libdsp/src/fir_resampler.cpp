...
#include <libdsp/fir_resampler.h>
...
InternalFirResampler::InternalFirResampler( int width, sample_t* impulses_ ) :
	width_( width ),
...
InternalFirResampler::~InternalFirResampler() { }

void InternalFirResampler::clear()
{
	imp_phase = 0;
	if ( buf.size() )
	{
...
blargg_err_t InternalFirResampler::buffer_size( int new_size )
{
...
double InternalFirResampler::time_ratio( double new_factor, double rolloff, double gain )
{
	ratio_ = new_factor;
...
int InternalFirResampler::input_needed( blargg_long output_count ) const
{
...
int InternalFirResampler::avail_( blargg_long input_count ) const
{
...
int InternalFirResampler::skip_input( long count )
{
